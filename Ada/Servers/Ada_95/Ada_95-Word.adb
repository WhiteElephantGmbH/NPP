...
package body Ada_95.Word is

  package Id is

    procedure Do_Add (Image : String) with Inline;

    function Handle_For (Image : String) return Handle;
  ...
  package Key is
    ...
    function Item_Of (Image : String) return Lexical.Name_Key;
  ...
  package body Id is
    ...
    type Tree_Access is access Word_Tree;
    for Tree_Access'storage_pool use Memory.Global_Pool.all;

    type Word_Tree is record
      Word  : Handle;
      Left  : Tree_Access;
      Right : Tree_Access;
    end record;
    ...
    type Table is array (Character) of Length_Table;
    ...
    function Comparison (Left  : String;
                         Right : String) return Result with Inline is
    begin
      for Index in Left'range loop
        declare
          Left_Character  : constant Character := Left(Index);
          Right_Character : constant Character := Right(Index + Right'first - Left'first);
        begin
          if Left_Character > Right_Character then
            return After;
          elsif Left_Character < Right_Character then
            return Before;
          end if;
        end;
      end loop;
    ...
    function Style_Of (Image : String) return Case_Style with Inline is
      function Is_Upper (The_Character : Character) return Boolean renames Ada.Characters.Handling.Is_Upper;
      function Is_Lower (The_Character : Character) return Boolean renames Ada.Characters.Handling.Is_Lower;
    begin
      if Is_Upper (Image(Image'first)) then
        for Second_Index in Image'first + 1 .. Image'last loop
          if Is_Lower (Image (Second_Index)) then
            for Index in Second_Index + 1 .. Image'last loop
              if Is_Upper (Image(Index)) then
                return Mixed_Case;
              end if;
            end loop;
            return Capitalized;
          elsif Is_Upper (Image (Second_Index)) then
            for Index in Second_Index + 1 .. Image'last loop
              if Is_Lower (Image(Index)) then
                return Mixed_Case;
              end if;
            end loop;
            return Upper_Case;
        ...
        return Upper_Case;
      elsif Is_Lower (Image(Image'first)) then
        for Index in Image'first + 1 .. Image'last loop
          if Is_Upper (Image(Index)) then
            return Mixed_Case;
          end if;
        end loop;
        ...
        for Second_Index in Image'first + 1 .. Image'last loop
          if Is_Upper (Image(Second_Index)) then
            for Index in Second_Index + 1 .. Image'last loop
              if Is_Lower (Image(Index)) then
                return Mixed_Case;
              end if;
            ...
            return Upper_Case;
          elsif Is_Lower (Image(Second_Index)) then
            for Index in Second_Index + 1 .. Image'last loop
              if Is_Upper (Image(Index)) then
                return Mixed_Case;
              end if;
    ...
    procedure Create (Tree  : out Tree_Access;
                      Image :     String) with Inline is
    begin
      The_Handle := new Item'(Length => Image'length,
                              Image  => Image,
                              Style  => Style_Of (Image),
                              Key    => Key.Item_Of (Image));
      Tree := new Word_Tree'(Word => The_Handle,
                             Left  => null,
                             Right => null);
    end Create;
    ...
    begin
      if Tree = null then
        ...
        case Comparison (Image, Tree.Word.Image) is
        when Equal =>
          The_Handle := Tree.Word;
          return;
        when Before =>
          Add (Image, Tree.Left);
        when After =>
    ...
    procedure Do_Add (Image : String) is
    begin
      if Image'length = 0 then
        The_Handle := new Item'(Length => 0,
                                Image  => "",
                                Style  => Undefined_Case,
                                Key    => Lexical.Null_Key);
      else
        declare
          Tree : Tree_Access renames The_Table(Image(Image'first))(Image'length);
        begin
          if Tree = null then
            Create (Tree, Image);
          elsif Image'length > 1 then
            case Comparison (Image, Tree.Word.Image) is
            when Equal =>
              The_Handle := Tree.Word;
            when Before =>
    ...
    function Handle_For (Image : String) return Handle is
    begin
      Do_Add (Image);
      return The_Handle;
    end Handle_For;
    ...
    begin
      for Start_Character in Character loop
        for Length in Length_Range loop
          The_Table(Start_Character)(Length) := null;
        end loop;
      end loop;
    end Do_Finalize;
  ...
  package body Key is
    ...
    type Word_Tree (Length : Natural) is record
    ...
    begin
      for Index in Left'range loop
        declare
          Left_Character  : constant Character := Lower_Case_Map(Left(Index));
          Right_Character : constant Character := Lower_Case_Map(Right(Index + Right'first - Left'first));
        begin
          if Left_Character /= Right_Character then
            if Left_Character > Right_Character then
              return After;
    ...
    begin
      ...
      Tree := new Word_Tree'(Length => Image'length,
                             Image  => Image,
                             Key    => The_Key,
                             Left   => null,
                             Right  => null);
    ...
    function Item_Of (Image : String) return Lexical.Name_Key is
  ...
  procedure Add (Image : String) renames Id.Do_Add;

  function Handle_Of (Image : String) return Handle renames Id.Handle_For;
